...
public OnDynamicObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= CreateDynamic3DTextLabel("Press '~k~~GROUP_CONTROL_BWD~' to use elevator", COLOR_RG, 1784.9822, -1302.0426, Z - 0.9, 4.0);
	...
	if(objectid == FireStationGarage[0] || objectid == FireStationGarage[1] || objectid == FireStationGarage[2] || objectid == FireStationGarage[3])
 	{
  		PlaySoundInArea(1154, X, Y, Z, 20.0);
    }
    if(objectid == FireStationGarage[0]) PlaySoundInArea(1154, 1587.95, -1638.22, 6.93, 20.0);
	return true;
}
...
public OnPlayerPressButton(playerid, buttonid)
{
    ...
    if(buttonid == FireStationButtons[4] || buttonid == FireStationButtons[5])
	{
		...
		{
			ShowPlayerDialog(playerid, DIALOG_FIRESTATIONGARAGE2, DIALOG_STYLE_LIST, "Garage Door's", "Open Garage Door 1\nOpen Garage Door 2\nOpen Garage Door 3\nOpen All Garage Doors\nClose All Garage Doors", "Open", "Exit");
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
		}
		else
		{
    ...
    if(buttonid == FireStationButtons[2] || buttonid == FireStationButtons[3])
	{
		if(IsAMedic(playerid))
		{
			MoveDynamicObject(FireStationDoor[0],1328.8198,-1311.0787,18.6714,2);
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
			SetTimer("FSDoorClose1", 2500, 0);
		}
		else
		{
			SendErrorMessage(playerid,"Access denied.");
    ...
    if(buttonid == FireStationButtons[0] || buttonid == FireStationButtons[1])
	{
		if(IsAMedic(playerid))
		{
			ShowPlayerDialog(playerid, DIALOG_FIRESTATIONGARAGE, DIALOG_STYLE_LIST, "Garage Door's", "Open Garage Door 1\nOpen Garage Door 2\nOpen Garage Door 3\nOpen All Garage Doors\nClose All Garage Doors", "Open", "Exit");
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
		}
		else
		{
    ...
    if(buttonid == HospitalLeftDoor[0] || buttonid == HospitalLeftDoor[1])
	{
		if(IsAMedic(playerid))
		{
			...
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
			SetTimer("HLeftCloseDoor", 2500, 0);
		}
		else
		{
			SendErrorMessage(playerid,"Access denied.");
    ...
    if(buttonid == HospitalRightDoor[0] || buttonid ==  HospitalRightDoor[1])
	{
		if(IsAMedic(playerid))
		{
			...
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
			SetTimer("HRightCloseDoor", 2500, 0);
		}
		else
		{
			SendErrorMessage(playerid,"Access denied.");
	...
	// New LSPD interior.
	if(buttonid == LSPDButtons[0] || buttonid == LSPDButtons[1])
	{
		if(IsACop(playerid))
		{
		    GetDynamicObjectPos(LSPDCellDoor[0], X, Y, Z);
	...
	if(buttonid == rightdoor)
	{
		if(IsACop(playerid))
		{
			MoveDynamicObject(rightmovedoor,1616.1085,1594.6868,9.7760,2);
			PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
			SetTimer("CloseRightDoor", 2500, 0);
		}
		else
		{
			SendErrorMessage(playerid,"Access denied.");
	...
	if(buttonid == leftdoor)
	{
		if(IsACop(playerid))
		{
	    	MoveDynamicObject(leftmovedoor,1616.2495, 1579.2811, 9.7760,2);
	    	PlayerPlaySound(playerid, 6400, 0.0, 0.0, 10.0);
			SetTimer("CloseLeftDoor", 2500, 0);
		}
		else
		{
			SendErrorMessage(playerid,"Access denied.");
	...
	{
		if(!IsADocGuard(playerid)) return SendErrorMessage(playerid," Access denied.");
		ShowDocPrisonControls(playerid, 0);
	}
	if(buttonid == SFPDHighCMDButton[0]) // Chief
	{
		if(PlayerInfo[playerid][pLeader] != 0) return SendErrorMessage(playerid," Access denied.");
		SFPDDoors(0, 1);
	...
	{
		if(!IsACop(playerid)) return SendErrorMessage(playerid," Access denied.");
		SFPDDoors(3, 1);
	...
	if(listid == SkinList)
	{
		if(response)
		{
			...
			if(PlayerInfo[playerid][pDonateRank] >= 2)
			{
				...
				return SendClientMessageEx(playerid, COLOR_RG, "VIP: You have changed your clothes for free.");
			}
			if(IsValidSkin(modelid) == 0)
			{
				if(PlayerInfo[playerid][mInventory][13])
				{
					...
					PlayerInfo[playerid][pModel] = modelid;
					SetPlayerSkin(playerid, modelid);
					return SendClientMessageEx(playerid, COLOR_RG, "You have used a Restricted Skin token to change your clothes.");
				}
...
//	RemoveVendingMachines(playerid);
    PlayAudioStreamForPlayer(playerid, "https://media.rg-rp.net/Heart%20of%20Los%20Santos%20-%20(official)%20ft.%20Harlow.mp3");
	...
	SendClientMessage( playerid, COLOR_WHITE, "SERVER: Welcome to Rebound Gaming Roleplay." );
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(gPlayerLogged{playerid} == 1)
	{
		...
		switch(reason)
		{
			case 0:
			{
				...
				else
				{
					...
					ProxDetector(30.0, playerid, string, COLOR_RG,COLOR_RG,COLOR_RG,COLOR_RG,COLOR_RG);
				}
		...
		if(EventKernel[EventRequest] == playerid)
		{
			...
			ABroadCast( COLOR_RG, "{AA3333}AdmWarning{FFFF00}: The player that was requesting an event has disconnected/crashed.", 4 );
		}
		...
		if(GotHit[playerid] > 0)
		{
			if(GetChased[playerid] != INVALID_PLAYER_ID)
			{
				...
				{
					SendClientMessageEx(GetChased[playerid], COLOR_RG, "Your hit has left the server.");
	...
	if(success)
	{
		...
		foreach(new i : Player)
		{
			...
			if(!strcmp(ip, pip, true))
			{
				...
				{
					CreateBan(INVALID_PLAYER_ID, PlayerInfo[i][pId], i, PlayerInfo[i][pIP], "Successful RCON Login - Not Lead Admin", 1337);

					format(szMiscArray, sizeof(szMiscArray), "{AA3333}AdmWarning{FFFF00}: %s(%s) has successfully logged into RCON without being a Lead Admin, and has been perma-banned.", GetPlayerNameEx(i), ip);
					ABroadCast(COLOR_YELLOW, szMiscArray, 2);
				}
			}
		}
...
public OnPlayerEnterCheckpoint(playerid)
{
    if(GetPVarInt(playerid, "EventToken") == 1)
	{
	    ...
	    if(EventRCPT[RCPIdCurrent[playerid]] == 3 && PlayerInfo[playerid][pHydration] < 60) {
			...
			if(PlayerInfo[playerid][pRacePlayerLaps] % 10 == 0) {
			    ...
			    SendClientMessageEx(playerid, COLOR_RG, "You have been given a Double EXP Token for completing 25 laps");
			}
	...
	if(GetPVarType(playerid,"DeliveringVehicleTime"))
	{
		...
		SendClientMessageEx(playerid, COLOR_RG, szMessage);
		PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
	...
	if(GetPVarInt(playerid, "TruckDeliver") > 0 && gPlayerCheckpointStatus[playerid] != CHECKPOINT_RETURNTRUCK)
	{
		...
		{
			SetPlayerCheckpoint(playerid, 2432.0771,-2114.6887,13.6101, 5);
			GameTextForPlayer(playerid, "~w~Waypoint set ~r~Willowfield Truck Depot", 5000, 1);
			SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Return to the Willowfield Truck Depot (see checkpoint on radar).");
		}
		else
		{
	...
	if(gPlayerCheckpointStatus[playerid] == CHECKPOINT_DELIVERY)
	{
		...
		new string[128];
		format(string, sizeof(string), "* You have delivered %s to %s. Return the truck to the Willowfield Truck Depot to collect your pay.", GetInventoryType(business), Businesses[business][bName]);
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, 2432.0771,-2114.6887,13.6101) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
			...
			case CHECKPOINT_RETURNTRUCK:
			{
				...
				{
				    distancepay = floatmul(GetDistanceBetweenPoints(2432.0771,-2114.6887,13.6101, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				...
				{
					if(strcmp(Points[i][Name], "Willowfield Truck Depot", true) == 0)
					{
						mypoint = i;
					}
				}
	...
	/*else if(IsKeyJustDown(128, newkeys, oldkeys))
	{
	...
	else if (IsKeyJustDown(KEY_FIRE, newkeys, oldkeys))
 	{
 	    ...
 	    if( PlayerInfo[playerid][pC4Used] == 1 )
 	    {
			    ...
			    if(IsPlayerInRangeOfPoint(GoChase[playerid], 12.0, GetPVarFloat(playerid, "DYN_C4_FLOAT_X"), GetPVarFloat(playerid, "DYN_C4_FLOAT_Y"), GetPVarFloat(playerid, "DYN_C4_FLOAT_Z")))
			    {
			        if(PlayerInfo[GoChase[playerid]][pHeadValue] >= 1)
					{
						if (IsAHitman(playerid))
						{
							...
							foreach(new i: Player) if(IsAHitman(i)) SendClientMessage(i, COLOR_RG, string);
							...
							SendClientMessageEx(GoChase[playerid], COLOR_RG, string);
			...
			PickUpC4(playerid);
			SendClientMessageEx(playerid, COLOR_RG, " Bomb has been detonated!");
			PlayerInfo[playerid][pC4Used] = 0;
			return 1;
 	    }
 	    if(GetPVarType(playerid, "MovingStretcher"))
...
/*
	...
	if(PlayerInfo[playerid][pAdmin] < 4 && PlayerInfo[playerid][pASM] < 1)
	{
		if(++CommandSpamTimes[playerid] >= 5) {
			...
			SendClientMessageEx(playerid, COLOR_RG, "You have been muted automatically for spamming. Please wait 10 seconds and try again.");
...
public OnPlayerText(playerid, text[])
{
		...
		else
		{
			if(GotProposedBy[playerid] != INVALID_PLAYER_ID)
			{
				if(IsPlayerConnected(GotProposedBy[playerid]))
				{
					...
					SendClientMessageEx(GotProposedBy[playerid], COLOR_RG, string);
					return 0;
				}
				else
				{
			...
			else if(ProposedTo[playerid] != INVALID_PLAYER_ID)
			{
				if(IsPlayerConnected(ProposedTo[playerid]))
				{
					...
					SendClientMessageEx(playerid, COLOR_RG, string);
					format(string, sizeof(string), "* %s didn't want to marry you, no 'yes' was said.",GetPlayerNameEx(playerid));
					SendClientMessageEx(ProposedTo[playerid], COLOR_RG, string);
					return 0;
				}
				else
				{
	...
	if(Mobile[playerid] != INVALID_PLAYER_ID)
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_RG,"There's nobody there!");
...
public OnPlayerModelSelectionEx(playerid, response, extraid, modelid, extralist_id) {
	...
	*/
	...
	if(extraid == DYNAMIC_LAW_CLOTHES)
	{
		if(response)
		{
			PlayerInfo[playerid][pModel] = modelid;
			SetPlayerSkin(playerid, modelid);
			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "You have changed your uniform.");
		}
		else
			return SendClientMessageEx(playerid, COLOR_GRAD2, "You exit the clothes selection menu.");
	}
