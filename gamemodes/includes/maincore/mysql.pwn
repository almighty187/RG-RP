		...
		case LOADUSERDATA_THREAD:
		{
			if(IsPlayerConnected(extraid))
			{
				...
				for(new row;row < rows;row++)
				{
					...
					cache_get_value_name_int(row, "ChatStyle" , value);
					SetPVarInt(extraid, "ChatStyle", value);
					...
					cache_get_value_name(row,  "PAdmPromBy", PlayerInfo[extraid][AdmPromBy]);
					cache_get_value_name(row,  "PAdmPromDate", PlayerInfo[extraid][AdmPromDate]);
					cache_get_value_name(row,  "PBannedBy", PlayerInfo[extraid][BannedBy]);
					cache_get_value_name(row,  "PBannedReason", PlayerInfo[extraid][BannedReason]);
					cache_get_value_name(row,  "PHLMadeBy", PlayerInfo[extraid][HLMadeBy]);
					cache_get_value_name(row,  "PHLMadeDate", PlayerInfo[extraid][HLMadeDate]);
					cache_get_value_name(row,  "PHLPromBy", PlayerInfo[extraid][HLPromBy]);
					cache_get_value_name(row,  "PHLPromDate", PlayerInfo[extraid][HLPromDate]);
					cache_get_value_name(row, "PAdmBanCount", szResult); PlayerInfo[extraid][AdmBanCount] = strval(szResult);
					cache_get_value_name(row, "PKickCount", szResult); PlayerInfo[extraid][KickCount] = strval(szResult);
...
// Added by N0FeaR
stock g_mysql_SaveAdministration(SqlPid)
{
    new query[1024];
	printf("Saving Staff Data %d", SqlPid+1);
	mysql_format(MainPipeline, query, sizeof(query), "UPDATE `accounts` SET ");
	mysql_format(MainPipeline, query, sizeof(query), "%s`AdminLevel` = '%d',", query, PlayerInfo[SqlPid][pAdminLevel]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`Helper` = '%d',", query, PlayerInfo[SqlPid][pHelper]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PAdmPromBy` = '%s',", query, PlayerInfo[SqlPid][AdmPromBy]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PAdmPromDate` = '%s',", query, PlayerInfo[SqlPid][AdmPromDate]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PBannedBy` = '%s',", query, PlayerInfo[SqlPid][BannedBy]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PBannedReason` = '%s',", query, PlayerInfo[SqlPid][BannedReason]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PHLMadeBy` = '%s',", query, PlayerInfo[SqlPid][HLMadeBy]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PHLMadeDate` = '%s',", query, PlayerInfo[SqlPid][HLMadeDate]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PHLPromBy` = '%s',", query, PlayerInfo[SqlPid][HLPromBy]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PHLPromDate` = '%s',", query, PlayerInfo[SqlPid][HLPromDate]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PAdmBanCount` = '%d',", query, PlayerInfo[SqlPid][AdmBanCount]);
	mysql_format(MainPipeline, query, sizeof(query), "%s`PKickCount` = '%d'", query, PlayerInfo[SqlPid][KickCount]);
	mysql_format(MainPipeline, query, sizeof(query), "%s WHERE `id` = %d", query, GetPlayerSQLId(SqlPid));
	mysql_tquery(MainPipeline, query, "OnQueryFinish", "i", SENDDATA_THREAD);
	printf("%s", query);
    return 1;
}
...
stock g_mysql_SaveAccount(playerid, string[] = "")
{
	...
	SavePlayerInteger(query, GetPlayerSQLId(playerid), "ChatStyle", GetPVarInt(playerid, "ChatStyle"));
	...
	SavePlayerString(query, GetPlayerSQLId(playerid), "PAdmPromBy", PlayerInfo[playerid][AdmPromBy]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PAdmPromDate", PlayerInfo[playerid][AdmPromDate]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PBannedBy", PlayerInfo[playerid][BannedBy]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PBannedReason", PlayerInfo[playerid][BannedReason]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PHLMadeBy", PlayerInfo[playerid][HLMadeBy]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PHLMadeDate", PlayerInfo[playerid][HLMadeDate]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PHLPromBy", PlayerInfo[playerid][HLPromBy]);
	SavePlayerString(query, GetPlayerSQLId(playerid), "PHLPromDate", PlayerInfo[playerid][HLPromDate]);
	...
	switch(iType) {
		...
		case GROUP_QUERY_LOCKERS: while(iIndex < iRows) {
			...
			if(arrGroupLockers[iGroup][iLocker][g_fLockerPos][0] != 0.0)
			{
				...
				CreateDynamicPickup(1275, 23, arrGroupLockers[iGroup][iLocker][g_fLockerPos][0], arrGroupLockers[iGroup][iLocker][g_fLockerPos][1], arrGroupLockers[iGroup][iLocker][g_fLockerPos][2], 15, arrGroupLockers[iGroup][iLocker][g_iLockerVW], -1);
			}
			iIndex++;
		...
		case GROUP_QUERY_LOAD: while(iIndex < iRows) {
			...
			cache_get_value_name(iIndex, "SlotHolder", arrGroupData[iIndex][g_SlotHolder], MAX_PLAYER_NAME);
			...
			cache_get_value_name_int(iIndex, "ArmsDealer", arrGroupData[iIndex][gArmsDealer]);
			cache_get_value_name_int(iIndex, "DrugDealer", arrGroupData[iIndex][gDrugDealer]);
			cache_get_value_name_float(iIndex, "Arms_X", arrGroupData[iIndex][gArmsX]);
			cache_get_value_name_float(iIndex, "Arms_Y", arrGroupData[iIndex][gArmsY]);
			cache_get_value_name_float(iIndex, "Arms_Z", arrGroupData[iIndex][gArmsZ]);
			cache_get_value_name_float(iIndex, "Arms_A", arrGroupData[iIndex][gArmsA]);
			cache_get_value_name_int(iIndex, "ArmsWorld", arrGroupData[iIndex][gArmsWorld]);
			cache_get_value_name_float(iIndex, "Drug_X", arrGroupData[iIndex][gDrugX]);
			cache_get_value_name_float(iIndex, "Drug_Y", arrGroupData[iIndex][gDrugY]);
			cache_get_value_name_float(iIndex, "Drug_Z", arrGroupData[iIndex][gDrugZ]);
			cache_get_value_name_float(iIndex, "Drug_A", arrGroupData[iIndex][gDrugA]);
			cache_get_value_name_int(iIndex, "DrugWorld", arrGroupData[iIndex][gDrugWorld]);
			cache_get_value_name_int(iIndex, "DrugPot", arrGroupData[iIndex][gDrugPot]);
			cache_get_value_name_int(iIndex, "DrugCrack", arrGroupData[iIndex][gDrugCrack]);
			cache_get_value_name_int(iIndex, "DrugMeth", arrGroupData[iIndex][gDrugMeth]);
			cache_get_value_name_int(iIndex, "DrugEcstasy", arrGroupData[iIndex][gDrugEcstasy]);
			cache_get_value_name_int(iIndex, "DrugHeroin", arrGroupData[iIndex][gDrugHeroin]);
			cache_get_value_name_int(iIndex, "Pot_Price", arrGroupData[iIndex][gDrugPrices][0]);
			cache_get_value_name_int(iIndex, "Crack_Price", arrGroupData[iIndex][gDrugPrices][1]);
			cache_get_value_name_int(iIndex, "Meth_Price", arrGroupData[iIndex][gDrugPrices][2]);
			...
			{
				format(szResult, sizeof(szResult), "Armsprice_%i", i);
				cache_get_value_name_int(iIndex, szResult, arrGroupData[iIndex][gArmsPrices][i]);
			}
			...
			if(arrGroupData[iIndex][gArmsDealer] && arrGroupData[iIndex][gArmsX] != 0.0 && arrGroupData[iIndex][gArmsY] != 0.0 && arrGroupData[iIndex][gArmsZ] != 0.0)
			{
				format(szResult, sizeof(szResult), "%s Arms Dealer\n{1FBDFF}Type /armsdealer {FFFF00}to use.", arrGroupData[iIndex][g_szGroupName]);
				arrGroupData[iIndex][gActors][0] = CreateActor(arrGroupData[iIndex][gArmsSkin], arrGroupData[iIndex][gArmsX], arrGroupData[iIndex][gArmsY], arrGroupData[iIndex][gArmsZ], arrGroupData[iIndex][gArmsA]);
				arrGroupData[iIndex][gText][0] = CreateDynamic3DTextLabel(szResult, arrGroupData[iIndex][g_hDutyColour] * 256 + 0xFF, arrGroupData[iIndex][gArmsX], arrGroupData[iIndex][gArmsY], arrGroupData[iIndex][gArmsZ] + 0.3, 10.0, .worldid = arrGroupData[iIndex][gArmsWorld]);
				SetActorVirtualWorld(arrGroupData[iIndex][gActors][0], arrGroupData[iIndex][gArmsWorld]);
				arrGroupData[iIndex][g_adMapIcon] = CreateDynamicMapIcon(arrGroupData[iIndex][gArmsX], arrGroupData[iIndex][gArmsY], arrGroupData[iIndex][gArmsZ], 6, 0, arrGroupData[iIndex][gArmsWorld], .streamdistance = 500.0, .style = MAPICON_GLOBAL);
			}
			if(arrGroupData[iIndex][gDrugDealer] && arrGroupData[iIndex][gDrugX] != 0.0 && arrGroupData[iIndex][gDrugY] != 0.0 && arrGroupData[iIndex][gDrugZ] != 0.0)
			{
				format(szResult, sizeof(szResult), "%s Drug Dealer\n{1FBDFF}Type /drugdealer {FFFF00}to use.", arrGroupData[iIndex][g_szGroupName]);
				arrGroupData[iIndex][gActors][1] = CreateActor(arrGroupData[iIndex][gDrugSkin], arrGroupData[iIndex][gDrugX], arrGroupData[iIndex][gDrugY], arrGroupData[iIndex][gDrugZ], arrGroupData[iIndex][gDrugA]);
				arrGroupData[iIndex][gText][1] = CreateDynamic3DTextLabel(szResult, arrGroupData[iIndex][g_hDutyColour] * 256 + 0xFF, arrGroupData[iIndex][gDrugX], arrGroupData[iIndex][gDrugY], arrGroupData[iIndex][gDrugZ] + 0.3, 10.0, .worldid = arrGroupData[iIndex][gDrugWorld]);
				SetActorVirtualWorld(arrGroupData[iIndex][gActors][1], arrGroupData[iIndex][gDrugWorld]);
				arrGroupData[iIndex][g_ddMapIcon] = CreateDynamicMapIcon(arrGroupData[iIndex][gDrugX], arrGroupData[iIndex][gDrugY], arrGroupData[iIndex][gDrugZ], 24, 0, arrGroupData[iIndex][gDrugWorld], .streamdistance = 500.0, .style = MAPICON_GLOBAL);
			}
			iIndex++;
		}
		...
		case GROUP_QUERY_ADDBAN: {
			...
			PlayerInfo[otherplayer][pSlotHolder] = INVALID_GROUP_ID;
