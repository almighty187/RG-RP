...
foreach(new i: Player)
{
		...
		if(1 <= GetPlayerState(i) <= 3) {
			if(playerTabbed[i] >= 1) {
				if(++playerTabbed[i] >= 9200 && PlayerInfo[i][pAdmin] < 2) {
					SendServerMessage(i, "You have been automatically kicked for alt-tabbing.");
					return Disconnect(i);
				}
			}
				...
				if(++playerAFK[i] >= 1200 && PlayerInfo[i][pAdmin] < 2) {
					SendServerMessage(i, "You have been automatically kicked for idling.");
					return Disconnect(i);
				}
			}
			else playerAFK[i] = 0;
		...
		{
			SendServerMessage(i, "You have been kicked for PaintBall Exploiting.");
