...
CMD:pnsedit(playerid, params[])
{
	...
	{
		SendSyntaxMessage(playerid, "/pnsedit [name] [id] [amount]");
...
CMD:pnsstatus(playerid, params[])
{
	...
	{
		SendSyntaxMessage(playerid, "/pnsstatus [id]");
		return 1;
	}
	if (PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pASM] >= 1)
	{
...
CMD:gotopaynspray(playerid, params[])
{
	...
	{
		new id;
		if(sscanf(params, "d", id)) return SendSyntaxMessage(playerid, "/gotopaynspray [id]");
