	    ...
	    if(PlayerInfo[playerid][pFamed] >= 1)
	    {
			...
			else if(strcmp(params,"trfamed",true) == 0)
			{
				if (GetPlayerState(playerid) == 2)
				{
					...
					fVehSpeed[playerid] = 0.0;
					SendServerMessage(playerid, "Famed: You have traveled to the front of the New Eire famed lounge.");
					SetPlayerInterior(playerid,0);
					PlayerInfo[playerid][pInt] = 0;
					SetPlayerVirtualWorld(playerid, 0);
					PlayerInfo[playerid][pVW] = 0;
...
CMD:v(playerid, params[]) {
	...
	if(PlayerInfo[playerid][pDonateRank] >= 1 || PlayerInfo[playerid][pAdmin] >= 2 || PlayerInfo[playerid][pVIPMod]) {
		if(isnull(params)) {
			...
			{
				format(szMessage, sizeof(szMessage), "Admin %s: %s", GetPlayerNameEx(playerid), params);
			}
