...
CMD:helpers(playerid, params[])
{
    ...
    {
        SendClientMessageEx(playerid, COLOR_GRAD1, "Helpers Online:");
        foreach(new i: Player)
		{
			...
			if(PlayerInfo[i][pHelper] >= 1) {
				...
				if(PlayerInfo[i][pHelper] == 2&&PlayerInfo[i][pAdmin]<2) {
					if(GetPVarInt(i, "HelperDuty") == 1) {
						...
						format(string, sizeof(string), "** Community Helper: %s	(Requests This Hour: %d | Requests Today: %d)", GetPlayerNameEx(i), ReportHourCount[i], ReportCount[i]);
					}
				}
				if(PlayerInfo[i][pHelper] == 3&&PlayerInfo[i][pAdmin]<2) {
					if(GetPVarInt(i, "HelperDuty") == 1) {
						...
						format(string, sizeof(string), "** Senior Helper: %s	(Requests This Hour: %d | Requests Today: %d)", GetPlayerNameEx(i), ReportHourCount[i], ReportCount[i]);
					}
				}
				if(PlayerInfo[i][pHelper] >= 4&&PlayerInfo[i][pAdmin]<2) {
					if(GetPVarInt(i, "HelperDuty") == 1) {
						...
						format(string, sizeof(string), "** Head Helper: %s	(Requests This Hour: %d | Requests Today: %d)", GetPlayerNameEx(i), ReportHourCount[i], ReportCount[i]);
					}
				}
				SendClientMessageEx(playerid, COLOR_GRAD2, string);
			}
        ...
        SendServerMessage(playerid, "If you have questions regarding gameplay, or the server use /newb.");
        SendServerMessage(playerid, "If you see suspicious happenings/players /report [id] [reason].");
    }
    return 1;
}
...
CMD:nonewbie(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] >= 3 || PlayerInfo[playerid][pHelper] >= 4)
	{
		...
		{
			nonewbie = 1;
			SendClientMessageToAllEx(COLOR_RG, "Newbie chat channel disabled by an Admin/Helper!");
		}
		else
		{
			nonewbie = 0;
...
CMD:checkrequestcount(playerid, params[])
{
    if(PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pASM] >= 1 || PlayerInfo[playerid][pHelper] >= 3 || PlayerInfo[playerid][pPR] > 0)
	{
		...
		if(sscanf(params, "s[24]s[11]", adminname, tdate)) return SendSyntaxMessage(playerid, "/checkrequestcount [helper name] [date (YYYY-MM-DD)]");
...
/*CMD:hlban(playerid, params[])
{
	if(IsPlayerConnected(playerid))
	{
		if(PlayerInfo[playerid][pAdmin] >= 2 || PlayerInfo[playerid][pHelper] >= 1)
		{
			...
			if(sscanf(params, "u", giveplayerid)) return SendSyntaxMessage(playerid, "/hlban [playerid/PartOfName]");
...
CMD:hl(playerid, params[])
{
	...
	if(isnull(params)) return SendSyntaxMessage(playerid, "(/newb)ie [text]");
...
CMD:hlkick(playerid, params[])
{
	if (PlayerInfo[playerid][pHelper] >= 1 || PlayerInfo[playerid][pAdmin] >= 1){
		new giveplayerid;
		if(sscanf(params, "u", giveplayerid)) return SendSyntaxMessage(playerid, "/hlkick [playerid/PartOfName]");
...
CMD:nunmute(playerid, params[])
{
	...
	{
		new string[128], giveplayerid;
		if(sscanf(params, "u", giveplayerid)) return SendSyntaxMessage(playerid, "/nunmute [playerid/PartOfName]");
...
CMD:nmute(playerid, params[])
{
	...
	{
		new string[128], giveplayerid;
		if(sscanf(params, "u", giveplayerid)) return SendSyntaxMessage(playerid, "/nmute [playerid/PartOfName]");
		...
		if(IsPlayerConnected(giveplayerid))
		{
			...
			else
			{
				...
				{
					SendErrorMessage(playerid, "That person is currently muted. You are unable to unmute players from the newbie chat as a Helper.");
				}
			}
		}
	}
...
CMD:makehelper(playerid, params[])
{
	if (PlayerInfo[playerid][pAdmin] >= 1337 || PlayerInfo[playerid][pPR] > 1 || PlayerInfo[playerid][pHelper] >= 3)
	{
		...
		if(IsPlayerConnected(giveplayerid))
		{
			...
			if(PlayerInfo[playerid][pHelper] == 4 && level > 2) return SendClientMessageEx(playerid, COLOR_GREY, "You can only promote players to Helper/Community Helper!");
			...
			switch(level)
			{
				...
				case 2:
				{
					format(string, sizeof(string), "You have been made a Community Helper by %s", GetPlayerNameEx(playerid));
					SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "You have given Community Helper to %s", GetPlayerNameEx(giveplayerid));
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "%s(%d) has been made a Community Helper by %s(%d)", GetPlayerNameEx(giveplayerid), GetPlayerSQLId(giveplayerid), GetPlayerNameEx(playerid), GetPlayerSQLId(playerid));
					Log("logs/admin.log", string);
				}
				case 3:
				{
					format(string, sizeof(string), "You have been promoted to Senior Helper by %s", GetPlayerNameEx(playerid));
					SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "You have promoted %s to Senior Helper", GetPlayerNameEx(giveplayerid));
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "%s(%d) has been made a Senior Helper by %s(%d)", GetPlayerNameEx(giveplayerid), GetPlayerSQLId(giveplayerid), GetPlayerNameEx(playerid), GetPlayerSQLId(playerid));
					Log("logs/admin.log", string);
				}
				case 4:
				{
					format(string, sizeof(string), "You have been promoted to Head Helper by %s", GetPlayerNameEx(playerid));
					SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "You have promoted %s to Head Helper", GetPlayerNameEx(giveplayerid));
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "%s(%d) has been made a Head Helper by %s(%d)", GetPlayerNameEx(giveplayerid), GetPlayerSQLId(giveplayerid), GetPlayerNameEx(playerid), GetPlayerSQLId(playerid));
					Log("logs/admin.log", string);
				}
			}
		}
	...
	if (PlayerInfo[playerid][pAdmin] >= 1337 || PlayerInfo[playerid][pHelper] >= 3 || PlayerInfo[playerid][pPR] >= 1)
	{
		...
		if(IsPlayerConnected(giveplayerid))
		{
		    if(PlayerInfo[playerid][pHelper] == 3 && PlayerInfo[giveplayerid][pHelper] != 1) {
		        SendErrorMessage(playerid, "You can only remove helpers.");
		        return 1;
		    }
			if(PlayerInfo[giveplayerid][pHelper] != 0)
			{
				...
				format(string, sizeof(string), "%s has kicked you out from the Helper team.", GetPlayerNameEx(playerid));
				SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, string);
				format(string, sizeof(string), "You took %s's Helper rank.", GetPlayerNameEx(giveplayerid));
				SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
				format(string, sizeof(string), "%s(%d) has had their Helper removed by %s(%d)", GetPlayerNameEx(giveplayerid), PlayerInfo[giveplayerid][pId], GetPlayerNameEx(playerid), PlayerInfo[playerid][pId]);
				Log("logs/admin.log", string);
...
CMD:requesthelp(playerid, params[])
{
	...
	{
		SendErrorMessage(playerid, "You are currently banned from requesting help.");
		return 1;
	}
	if(JustReported[playerid] > 0)
	{
		SendErrorMessage(playerid, "Wait 10 seconds after sending a next request!");
		return 1;
	}
	...
	SendDutyAdvisorMessage(TEAM_AZTECAS_COLOR, string);
	SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You have requested help from a Helper, wait for a reply.");
...
CMD:rhmute(playerid, params[])
{
	...
	{
		new string[128], giveplayerid;
		if(sscanf(params, "u", giveplayerid)) return SendSyntaxMessage(playerid, "/rhmute [playerid/PartOfName]");
		...
		if(IsPlayerConnected(giveplayerid))
		{
			if(PlayerInfo[giveplayerid][pRHMuteTime] == 0)
			{
			    if(PlayerInfo[giveplayerid][pRHMutes] == 0)
			    {
					...
					ShowPlayerDialogEx(giveplayerid, 7954, DIALOG_STYLE_MSGBOX, "Help request abuse warning", "A Helper has warned you not to abuse /requesthelp.\n\nNote that future abuse of /requesthelp could result in a mute from /requesthelp or loss of that privilege altogether.", "Next", "");
...
CMD:rhmutereset(playerid, params[])
{
	if (PlayerInfo[playerid][pHelper] >= 2)
	{
		...
		if(sscanf(params, "us[64]", giveplayerid, reason)) return SendSyntaxMessage(playerid, "/rhmutereset [playerid/PartOfName] [reason]");
	    ...
	    if(!GetPVarType(playerid, "HelpingSomeone")) {
     		foreach(new x: Player) {
				...
				if(PlayerInfo[x][pLevel] == 1 && PlayerInfo[x][pHelpedBefore] == 0 && PlayerInfo[x][pAdmin] == 0 && PlayerInfo[x][pId] != PlayerInfo[playerid][pId]) {
					...
					ShowPlayerDialogEx(x, 0, DIALOG_STYLE_MSGBOX, "Helper Alert", "A Helper has just teleported to you. Feel free to ask him anything related to Rebound Gaming that you may have issues/concerns with.", "Close", "");
...
CMD:accepthelp(playerid, params[])
{
    if(PlayerInfo[playerid][pHelper] < 2) {
		...
		if(sscanf(params, "u", Player)) {
			SendSyntaxMessage(playerid, "/accepthelp [PlayerID]");
		}
		else if(Player == playerid) {
		    SendErrorMessage(playerid, "You can't accept a help request from yourself.");
		}
...
CMD:togha(playerid, params[])
{
	if(PlayerInfo[playerid][pHelper] < 2 && PlayerInfo[playerid][pAdmin] < 2) return SendErrorMessage(playerid, "You're not authorized to use this CMD.");
	if(GetPVarInt(playerid, "CHChat") == 1)
	{
		PlayerInfo[playerid][pToggledChats][16] = 1;
		SendServerMessage(playerid, "You have disabled Helper chat.");
		return SetPVarInt(playerid, "CHChat", 0);
	}
	else
	{
		PlayerInfo[playerid][pToggledChats][16] = 0;
		SendServerMessage(playerid, "You have enabled Helper chat.");
		return SetPVarInt(playerid, "CHChat", 1);
	}
}
