...
hook OnGameModeInit() {
	...
	CreateDynamic3DTextLabel("{FF8000}/loadforklift\nTo load the crate onto your forklift", COLOR_TWGREEN, 134.7094,-4380.9165,52.603, 20.0, _, _, -1, -1, -1, _, 20.0);
	CreateDynamic3DTextLabel("{FF8000}LOCKDOWN\nPUSH ONLY IN CASE OF EMERGENCY", COLOR_TWGREEN, -1121.70, 4290.41, 5.88, 20.0, _, _, 1, 0, 0, _, 20.0);

	// General text labels
	/*
	CreateDynamic3DTextLabel("All Saints Plaza Parking Garage\nType /enter with your vehicle to enter",COLOR_TWGREEN,1218.69,-1425.01,13.15+0.6,25.0);
	CreateDynamic3DTextLabel("Club VIP Parking\nType /enter with your vehicle to enter",COLOR_TWGREEN,1814.6857,-1559.2028,13.4834+0.4,15.0);
	CreateDynamic3DTextLabel("Gym Parking Garage\nType /enter with your vehicle to enter",COLOR_TWGREEN,2273.4026,-1724.3064,13.5469+0.6,25.0);
	CreateDynamic3DTextLabel("Courthouse Parking Garage\nType /enter with your vehicle to enter",COLOR_TWGREEN,1418.5190,-1663.8959,13.5395+0.6,25.0);
	*/
    CreateDynamic3DTextLabel("To change your number type /vipnum",COLOR_TWGREEN,2549.548095, 1404.047729, 7699.584472+0.6,4.0);//
    CreateDynamic3DTextLabel("To change your number type /vipnum",COLOR_TWGREEN,1832.6000, 1375.1700, 1464.4600+0.6,4.0);//
    CreateDynamic3DTextLabel("Type /travel\nTo exit the VIP garage with your vehicle",COLOR_TWGREEN,-4429.944824, 905.032470, 987.078186+0.5,13.0);// VIP garage travel
    CreateDynamic3DTextLabel("/viplocker\nTo open the VIP locker.",COLOR_TWGREEN,2555.747314, 1404.106079, 7699.584472+0.6,4.0);/// LS VIP locker room
	CreateDynamic3DTextLabel("/viplocker\nTo open the VIP locker.",COLOR_TWGREEN,1832.0533, 1380.7281, 1464.3822+0.6,4.0);/// LV Main VIP locker room
    CreateDynamic3DTextLabel("/viplocker\nTo open the VIP locker.",COLOR_TWGREEN,772.4844, 1715.7213, 1938.0391+0.6,4.0);/// LV Plat VIP locker room
	CreateDynamic3DTextLabel("/getgift\nTo reach inside.",COLOR_TWGREEN,2546.611572, 1403.323608, 7700.768066,8.0);/// VIP Lounge
	CreateDynamic3DTextLabel("/getgift\nTo reach inside.",COLOR_TWGREEN,1726.1000, 1370.1000, 1449.9000,8.0);/// LV VIP Lounge
	CreateDynamic3DTextLabel("/getgift\nTo reach inside.",COLOR_TWGREEN,1763.5000, 1432.5000, 2015.7000,8.0);/// LV GVIP Lounge
	CreateDynamic3DTextLabel("/getgift\nTo reach inside.",COLOR_TWGREEN,772.4000, 1743.2000, 1938.8800,8.0);/// LV PVIP Lounge
	...
	*/
    ...
    //CreateDynamic3DTextLabel("Type /getmats to purchase material packages",COLOR_TWGREEN,-1816.528686, -179.502624, 9.398437+0.6,5.0);
	...
	CreateDynamic3DTextLabel("Name Change Point \nType /changename to change your name",COLOR_TWGREEN,-2279.6545, 2311.2238, 4.9641+0.6,18.0); // Namechange (TR)

	CreateDynamic3DTextLabel("Famed Locker \nType /famedlocker to access the locker", COLOR_TWGREEN,901.4825,1429.7404,-82.3235+0.6,4.0); //Famed Locker
	...
	//CreateDynamic3DTextLabel("Drug Run\nType /getcrate here to get meth & ecsasy", COLOR_TWGREEN, -1446.8916,1503.4746,1.7366, 10.0);
