	...
	if(!GetPVarType(playerid, "IsInArena"))  // Check if player is not in arena
	{
		if(!pCBugging[playerid] && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
		{
			if(PRESSED(KEY_FIRE))
			{
				...
				{
					case WEAPON_DEAGLE, WEAPON_SHOTGUN, WEAPON_SNIPER, WEAPON_SILENCED, WEAPON_SAWEDOFF, WEAPON_SHOTGSPA, WEAPON_UZI, WEAPON_MP5, WEAPON_COLT45, WEAPON_AK47, WEAPON_M4, WEAPON_TEC9, WEAPON_RIFLE :
					{
