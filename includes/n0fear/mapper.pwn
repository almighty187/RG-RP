...
#define MAX_MAP_OBJ 5000
#define SELECTMODE_MAP 3
#define EDITOR_MODE_MAPFURN_CREATE 1
#define EDITOR_MODE_MAPFURN_EDIT 2
...
new ObjectList[][e_ObjectList] =
{
	...
	{11751, "AreaBoundary50m"}, {11752, "AreaBoundary10m"}, {11753, "AreaBoundary1m"}, {-2000, "Dufflebag"}, {-2001, "Dufflebag2"},
	{-2002, "PoliceKevlar"}, {-2003, "pilotglasses"}, {-2005, "LA Dodgers Cap"}, {-2006, "New York Yankees Cap"}, {-2007, "Atlanta Braves Cap"},
	{-2008, "Sox Cap"}, {-2009, "DC Bag"}, {-2010, "Gucci Bag"}, {-2011, "Playstion 5"}, {-2012, "Nintendo 64"}, {-2013, "Playstion"},
	{-2014, "Playstion 2 Fat"}, {-2015, "Playstion 2 Slim"}, {-2016, "Playstion 3 Fat"}, {-2017, "Playstion 3 Slim"}, {-2018, "Playstion 4"},
	{-2019, "Super Nintendo Entertainment System"}, {-2020, "Nintendo Wii"}, {-2021, "Xbox"}, {-2022, "Xbox360 Fat"} , {-2023, "Xbox360 Slim"},
	{-2024, "Xbox One"}, {-2025, "TV And Shelf"}, {-2026, "Police Kevlar2"}, {-2027, "Paramedic Kevlar"}, {-20030, "Sprunk Can"}, {-20032, "Suit Case"},
	{-2026, "Military Backpack"}, {-2027, "Military Backpack 2"}, {-2028, "Tv"}, {-20028, "Cola Vending"}, {-20029, "Sprunk Vending"},
	{-20031, "Gas Pump"}, {-20034, "Buss Stop"}, {-2030, "Pizza Stack"}, {-2031, "Binco"}, {-2034, "Seven Eleven"}, {-2035, "Seven Eleven2"},
	{-2036, "Seven Eleven Door Left"}, {-2037, "Seven Eleven Door Right"}
};
...
new FurnObjs2[701] = {
	...
	11714,11715,11716,11706,11705,11704,19786,19787,-2011,
...
CMD:mapper(playerid, params[])
{
	...
	if(GetPVarInt(playerid, "Mapper") < 1 && PlayerInfo[playerid][pAdminLevel] < 1338) return SendErrorMessage(playerid, "You aren't a mapper.");
	new param[64], amount, val;
	if(sscanf(params, "s[64]I(-1)I(-1)", param, amount, val)) {
		SendClientMessageEx(playerid, COLOR_GREEN, "/map [usage]");
		SendClientMessageEx(playerid, COLOR_GREY, "  plant | edit | mine |");
		if(GetPVarInt(playerid, "Mapper") > 1 || PlayerInfo[playerid][pAdminLevel] > 1338) {
			SendClientMessageEx(playerid, COLOR_GREY, " addmapper | removemapper |");
		}
		return 1;
	}
	if (strcmp(param, "plant", true) == 0) {
		if (amount == (-1)) {
			SendClientMessage(playerid, -1, "Displaying category selection dialog.");
			ShowPlayerDialog(playerid, 522, DIALOG_STYLE_LIST, "Category Selection", "Weapons\nNature\nCar Parts\nFun Stuff\nRoads\nBarriers\nVarious\nMisc\nHouse-style List\nCustom Objects", "Select", "Exit");
    	}
		else
		{
			//if(amount < 320 || amount > 21000) return SendClientMessageEx(playerid, -1, "Invalid Object ID!");
			new found = 0, foundid;
			...
			if(found == 0) return SendClientMessageEx(playerid, -1, "Invalid Object ID!");
	...
	{
		if(PlayerInfo[playerid][pAdminLevel] < 1338 && GetPVarInt(playerid, "Mapper") < 2) return SendErrorMessage(playerid, "Insufficient rank!");
		if(amount == (-1)) {
			SendClientMessage(playerid, COLOR_GREEN, "USAGE: /map addmapper {FFFFFF}[playerid]");
			return 1;
		}
		if(amount == playerid) return SendErrorMessage(playerid, "You can't add yourself as a mapper.");
		if(IsPlayerNPC(amount)) return SendErrorMessage(playerid, "You cant do this to NPCs.");
		if(!IsPlayerConnected(amount)) return SendErrorMessage(playerid, "Player doesn't exist!");
		if(val < 1 || val > 2) return SendErrorMessage(playerid, "You can only make a mapper rank 1(Mapper) or rank 2(Lead Mapper).");
		if(val == 2 && PlayerInfo[playerid][pAdminLevel] < 1338) return SendErrorMessage(playerid, "Only the highest rank administrators can add lead-mappers.");
		//SetPDataInt(amount, "Mapper", val);
		new string[128];
		format(string, sizeof(string), "You've been made a rank %d mapper by %s.", val, GetPlayerNameEx(playerid));
		SendClientMessageEx(amount, COLOR_WHITE, string);
		format(string, sizeof(string), "You've made %s a rank %d mapper.", GetPlayerNameEx(amount), val);
	...
	else if(strcmp(param, "removemapper", true) == 0)
	{
		...
		new string[128];
		format(string, sizeof(string), "You've been removed from the mapping-team by %s.", GetPlayerNameEx(playerid));
		SendClientMessageEx(amount, COLOR_WHITE, string);
		format(string, sizeof(string), "You've removed %s from the mapping-team.", GetPlayerNameEx(amount));
		SendClientMessageEx(playerid, COLOR_WHITE, string);
	}
	else if(strcmp(param, "mine", true) == 0)
	{
		GetMapping(playerid,GetPlayerNameEx(playerid),1);
	}
	else if(strcmp(param, "select", true) == 0)
	{
		SetPVarInt(playerid, "SelectMode", SELECTMODE_MAP);
		SelectObject(playerid);
		SendClientMessage(playerid, COLOR_WHITE, "[TIP] {FFFFFF}Click on a map object to edit it.");
	}
    ...
    switch(dialogid)
	{
	    case 522:
		{
			...
			switch(listitem)
			{
				...
				case 8:
				{
					new oblist[sizeof(FurnObjs2)];
					for(new i = 0; i < sizeof(FurnObjs2); i++)
					{
						oblist[i] = FurnObjs2[i];
					}
					ShowModelSelectionMenuEx(playerid, oblist, sizeof(FurnObjs2), "Furniture List", 13, 16.0, 0.0, -55.0);
				}
				case 9: // 0.3DL Objects
				{
					new oblist[25] = {
						-20028, -20029, -20031, -20034, -2030, -2031,
						-2034, -2035, -2036, -2037
					};
					ShowModelSelectionMenuEx(playerid, oblist, sizeof(oblist), "Custom Objects", 13, 16.0, 0.0, -55.0);
				}
	        }
	    }
		case 518:
...
/*hook OnPlayerEditObject(playerid, playerobject, objectid, response, Float:fX, Float:fY, Float:fZ, Float:fRotX, Float:fRotY, Float:fRotZ)
{
    ...
    switch(GetPVarInt(playerid, "EditorMode"))
	{
		case EDITOR_MODE_MAPFURN_EDIT: // Map Editor
		{
			...
			MapInfo[h][mY2]=fY;
			MapInfo[h][mZ2]=fZ;
			MapInfo[h][mrX2]=fRotX;
			MapInfo[h][mrY2]=fRotY;
			MapInfo[h][mrZ2]=fRotZ;
			//SaveMapObj(h);
  		}
	}
	return 1;
...
hook OnPlayerEditObject(playerid, playerobject, objectid, response, Float:fX, Float:fY, Float:fZ, Float:fRotX, Float:fRotY, Float:fRotZ)
{
    // If the response is neither final nor cancel, return true
    if(response != EDIT_RESPONSE_FINAL && response != EDIT_RESPONSE_CANCEL)
    {
        return true;
    }

    // Destroy the player's current furniture object if it exists
    if(GetPVarInt(playerid, "FurnObject") != 0)
    {
        DestroyPlayerObject(playerid, GetPVarInt(playerid, "FurnObject"));
    }

    // Check if the player is in Editor Mode
    if(GetPVarInt(playerid, "EditorMode"))
    {
        // Use a switch statement for different editor modes
        switch(GetPVarInt(playerid, "EditorMode"))
        {
            case EDITOR_MODE_MAPFURN_EDIT: // Map Editor
            {
                ...
                // Make the player controllable again
                TogglePlayerControllable(playerid, true);

                // If the response is final
                if(response == EDIT_RESPONSE_FINAL)
                {
                    // Get the handle of the furniture being edited
                    new h = GetPVarInt(playerid, "FurnEditH");

                    // Move the dynamic object to the new position and rotation
                    MoveDynamicObject(MapInfo[h][mObject], fX, fY, fZ, 50.0, fRotX, fRotY, fRotZ);

                    // Update the map information with the new position and rotation
                    MapInfo[h][mX2] = fX;
                    MapInfo[h][mY2] = fY;
                    MapInfo[h][mZ2] = fZ;
                    MapInfo[h][mrX2] = fRotX;
    ...
    return 1; // Always return 1 at the end of the function*/
}
hook OnPlayerModelSelectionEx(playerid, response, extraid, modelid) {
    SendClientMessage(playerid, -1, "OnPlayerModelSelectionEx hook called.");
    if (!response) {
        return SendClientMessageEx(playerid, -1, "Menu cancelled.");
    }
    if (extraid == 1) { // MAP OBJECTS
        SendClientMessage(playerid, -1, "Extraid is 1. Processing model selection.");
        new found = 0;
        for (new i = 0; i < MAX_OBJECT_ARRAY; i++) {
...
hook OnPlayerModelSelectionEx(playerid, response, extraid, modelid)
{
    if(!response) return SendClientMessageEx(playerid, -1, "Menu cancelled.");
	if(extraid == 1) //MAP OBJECTS
    {
		new found = 0;
		for(new i = 0; i < MAX_OBJECT_ARRAY; i++)
...
stock GetCloseMapObject(playerid)
{
    ...
    for(new h = 0; h < MAX_MAP_OBJ; h++)
    {
		...
		{
		    if(IsPlayerInRangeOfPoint(playerid, 2.5, MapInfo[h][mX2], MapInfo[h][mY2], MapInfo[h][mZ2]))
		    {
	...
	if(found == 0) {
		SendClientMessageEx(playerid, COLOR_LIGHTRED, "ERROR: Can't find any object!");
	} else ShowModelSelectionMenuEx(playerid, furnitob, found, "Select item", 11, 16.0, 0.0, -55.0);
    return 1;
}
stock GetMapping(playerid,name[],mine=0)
{
	if(PlayerInfo[playerid][pAdminLevel] < 1338 && GetPVarInt(playerid, "Mapper") < 1) return 1;
	...
	if(found == 0) return SendClientMessageEx(playerid, COLOR_LIGHTRED, "ERROR: Can't find any mapping for that player!");
	if(mine == 1)
	{
...
{
	if(GetPVarInt(playerid, "HouseKey") != 0 && (GetPlayerVirtualWorld(playerid) == GetPVarInt(playerid, "HouseKey"))) return SendErrorMessage(playerid, "You can't spawn objects in your house using '/map'.");
	if(objectid == 6553600) return SendErrorMessage(playerid, "Couldn't create map object: Invalid Object-ID.");
    ...
    for(new h = 0; h < MAX_MAP_OBJ; h++)
    {
        if(MapInfo[h][mID] == 0)
		{
			...
			MapInfo[h][mVW] = vw;
			format(MapInfo[h][mOwner], 25, "%s", GetPlayerNameEx(playerid));
			...
			new query[524];
			mysql_format(MainPipeline, query, sizeof(query), "INSERT INTO mapfurn (Owner, Model, PosX, PosY, PosZ, AngX, AngY, AngZ, Interior) VALUES ('%s', %i, %f, %f, %f, %f, %f, %f, %i)",
				MapInfo[h][mOwner],
				MapInfo[h][mID],
				MapInfo[h][mX2],
				MapInfo[h][mY2],
				...
				MapInfo[h][mrZ2],
				MapInfo[h][mInt]);
			mysql_tquery(MainPipeline, query, "OnBuildMapObject", "i", h);
            Streamer_Update(playerid);
			break;
        }
    }
