	...
	if(newkeys & KEY_YES)
	{
	    for (new i = 0; i < sizeof(DropInfo); i++)
    	{
			if (DropInfo[i][IsUsed] && IsPlayerInRangeOfPoint(playerid, 2.0, DropInfo[i][DropGunPosX], DropInfo[i][DropGunPosY], DropInfo[i][DropGunPosZ]))
	        {
	            ...
	            {
	            	if(PlayerInfo[playerid][pWRestricted] != 0 || PlayerInfo[playerid][pConnectHours] < 1) return SendClientMessageEx(playerid, COLOR_GRAD1, "You cannot use this while having a weapon restriction.");
