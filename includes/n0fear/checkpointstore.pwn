...
//----------------------------------------------------------
new Float:CheckpointAreas[13][6] =
{
	...
	{372.1912,-133.4454,1002.3047,372.7783,-118.8684,1002.1565}, // Pizza
 	...
 	{-100.3581,-25.1126,1001.4091,-104.7217,-10.8734,1001.5079} // Sex Shop

};
//----------------------------------------------------------
new CheckpointAreaType[13][1] =
{
	{CHECKPOINT_STORE},
	{CHECKPOINT_STORE},
	{CHECKPOINT_STORE},
	...
	switch(GetPVarInt(playerid, "Checkpoint"))
	{
   		...
   		case CHECKPOINT_SEXSHOP:
        {
            if(GetPVarInt(playerid, "OCC") == 0)
			{
				...
				switch(rand)
				{
				    case 1: SendClientMessage(playerid,COLOR_WHITE,"Cashier Says: {FFFFFF}Welcome, what can I get for you today?");
				    case 2: SendClientMessage(playerid,COLOR_WHITE,"Cashier Says: {FFFFFF}Hey, how are you?");
				    case 3: SendClientMessage(playerid,COLOR_WHITE,"Cashier Says: {FFFFFF}Welcome.");
				    case 4: SendClientMessage(playerid,COLOR_WHITE,"Cashier Says:{FFFFFF} Hello there, what can I do for you today?");
				    default: SendClientMessage(playerid,COLOR_WHITE,"Cashier Says: {FFFFFF}There's a variety of dildos to purchase.");
				}
				new iBusiness = InBusiness(playerid);
				if (Businesses[iBusiness][bInventory] < 1)
				{
				...
				for (new item; item < sizeof(SexItems); item++)
				{
					new cost = (PlayerInfo[playerid][pDonateRank] >= 1) ? (floatround(Businesses[iBusiness][bItemPrices][item] * 0.8)) : (Businesses[iBusiness][bItemPrices][item]);
				...
				{
			    	ShowPlayerDialogEx(playerid, SHOPMENU, DIALOG_STYLE_LIST, "Menu", szDialog, "Buy", "Cancel");
			    	SetPVarInt(playerid, "OCC", 1);
   				}
   			}
   		}
