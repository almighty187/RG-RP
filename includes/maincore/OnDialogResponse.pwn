...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid != iLastDialogID[playerid]) {
		if(dialogid != DIALOG_FS_ELEVATOR1 && dialogid != DIALOG_FS_ELEVATOR2) { // For dialogs called from filterscripts.
			if(PlayerInfo[playerid][pAdmin] == 1338 || dialogid == 32700 || dialogid == 0 || dialogid == 522) return 1;
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	if(dialogid == DIALOG_DISABLED) return ShowPlayerDialogEx(playerid, DIALOG_DISABLED, DIALOG_STYLE_MSGBOX, "Account Disabled - Visit http://forum.rg-rp.org", "Your account has been disabled as it has been inactive for more than six months.\nPlease visit the forums and post an Administrative Request to begin the process to reactivate your account.", "Okay", "");
		...
		if(dialogid == REGISTERSEX)
		{
			if(response)
			{
				if(listitem == 0)
				{
					...
					SendServerMessage(playerid, "Alright, so you're a male.");
				...
				else
				{
					...
					SendServerMessage(playerid, "Alright, so you're a female.");
					ShowPlayerDialogEx(playerid, REGISTERMONTH, DIALOG_STYLE_LIST, "{FF0000}Which month was your character born?", "January\nFebruary\nMarch\nApril\nMay\nJune\nJuly\nAugust\nSeptember\nOctober\nNovember\nDecember", "Submit", "");
					RegistrationStep[playerid] = 2;
				}
			}
	...
	if(RegistrationStep[playerid] != 0 || strcmp(PlayerInfo[playerid][pBirthDate], "0000-00-00", true) == 0)
	{
		...
		else if(dialogid == REGISTERREF)
		{
			if(response)
			{
				...
				SendServerMessage(playerid, "Thanks for filling in all the information, now you can proceed to the tutorial!");
	...
	switch(dialogid)
	{
		case DIALOG_911PICKLOCK: if(response)
		{
			...
			else if(PlayerVehicleInfo[playerid][listitem][pvImpounded]) SendErrorMessage(playerid, "You can not report an impounded vehicle. If you wish to reclaim it, do so at the DMV.");
			else if(PlayerVehicleInfo[playerid][listitem][pvDisabled] == 1) SendErrorMessage(playerid, "You can not report a disabled vehicle. It is disabled due to your VIP level (vehicle restrictions).");
			else if(PlayerVehicleInfo[playerid][listitem][pvSpawned] == 0) SendErrorMessage(playerid, "You can not report a stored vehicle. Use /vstorage to spawn it.");
		...
		case DIALOG_NATION_CHECK:
		{
			...
			{
				SendServerMessage(playerid, "You did not provide a response, picking a random nation for you...");
				new rand = random(2);
		...
		case BIGEARS3:
		{
			if(response) {
				...
				{
					SendErrorMessage(playerid, "Only Senior Admins+ are allowed to use this feature.");
					return 1;
				}
		...
		case BIGEARS: if(response) switch(listitem) {
			...
			case 7: {
				...
				rBigEarT[playerid] = 0;
				SendServerMessage(playerid, "You have disabled the bigears feature, you no longer see anything on your screen.");
			}
		}
		case BIGEARS4: {
			if(response) {
				...
				SendServerMessage(playerid, "You can now see all the messages from this player.");
			}
			else ShowPlayerDialogEx(playerid, BIGEARS, DIALOG_STYLE_LIST, "Please choose an item to proceed", "Global Chat\nOOC Chat\nIC Chat\nFaction Chat\nGroup OOC Chat\nPlayer\nPrivate Messages\nDisable Bigears", "Select", "Cancel");
		}
		case BIGEARS5: {
		...
		case BIGEARS2: {
			if(response) {
				...
				if (arrGroupData[group][g_iGroupType] == GROUP_TYPE_CONTRACT && PlayerInfo[playerid][pAdmin] < 4 && PlayerInfo[playerid][pASM] < 1)
				{
					SendServerMessage(playerid, "Only Senior Admins+ are allowed to use this feature.");
					return 1;
				}
		...
		case DIALOG_DELETECAR:
		{
			if(response)
			{
				if(GetPVarType(playerid, "vDel")) {
					...
					return SendServerMessage(playerid, "Your vehicle has been permanently deleted.");
				}
	...
	if(dialogid == RCPCHOOSE)
	{
		if (response && ConfigEventCPs[playerid][0] == 1)
		{
			...
			{
				if(ConfigEventCPs[playerid][1] != 0) return SendServerMessage(playerid, "You cannot create a new checkpoint since you are editing an existing one.");
				if(ConfigEventCPId[playerid] >= 20) {
					ConfigEventCPs[playerid][0] = 0;
					ConfigEventCPs[playerid][1] = 0;
					ConfigEventCPId[playerid] = 0;
					return SendServerMessage(playerid, "You cannot create a new checkpoint since you have reached the checkpoint limit(20).");
				}
				new i;
				for(i = 0; i < 20; i++)
				{
				...
				SendServerMessage(playerid, "You are now creating a new checkpoint, you need to choose the position where the checkpoint will be at.");
				SendServerMessage(playerid, "NOTE: Once you are done and have the right place please press the fire button to save the position.");
				SendServerMessage(playerid, "NOTE: You can also cancel this action by pressing the AIM button.");
			}
			else if(listitem == 1) // Edit an existing checkpoint IN PROCESS
			{
				if(ConfigEventCPs[playerid][1] != 0) return SendServerMessage(playerid, "You cannot edit a checkpoint since you are editing an existing one.");
	...
	if((dialogid == EDITTOYS) && response)
	{
		...
		{
			SendClientMessageEx(playerid, COLOR_WHITE, "{AA3333}HINT:{33AA33} Position your camera differently to better see where your editing.");
			SetPVarInt(playerid, "ToySlot", listitem);
	...
	if((dialogid == EDITTOYS2)) {
		if(response) switch(listitem) {
			...
			case 1:
			{
				...
				SendClientMessage(playerid, COLOR_WHITE, "HINT: Hold {AA3333}~k~~PED_SPRINT~ {33AA33}to move your camera, press escape to cancel");
			}
			case 2:
			{
				new szstring[128];
	...
	if(dialogid == VIPNUMMENU)
	{
		if(response)
		{
			...
			if(strlen(numb) == 3)
			{
				...
				else if(GetPlayerCash(playerid) >= 5000 && GetPlayerCash(playerid) < 10000)
				{
					SetPVarInt(playerid, "WantedPh", numberstr);
					SetPVarInt(playerid, "CurrentPh", PlayerInfo[playerid][pPnumber]);
					SetPVarInt(playerid, "PhChangeCost", 5000);
					mysql_format(MainPipeline, query, sizeof(query), "SELECT `Username` FROM `accounts` WHERE `PhoneNr` = '%d'",numberstr);
					mysql_tquery(MainPipeline, query, "OnPhoneNumberCheck", "ii", playerid, 1);
					return 1;
				}
			...
			else if(strlen(numb) >= 5 && strlen(numb) <= 9)
			{
				...
				else if(GetPlayerCash(playerid) >= 2500 && GetPlayerCash(playerid) < 5000)
				{
					SetPVarInt(playerid, "WantedPh", numberstr);
					SetPVarInt(playerid, "CurrentPh", PlayerInfo[playerid][pPnumber]);
					SetPVarInt(playerid, "PhChangeCost", 5000);
	...
	if(dialogid == ADMIN_VEHCHECK && response) {
		...
		new query[60];
		mysql_format(MainPipeline, query, sizeof(query), "DELETE FROM `vehicles` WHERE `id` = '%d'", PlayerVehicleInfo[giveplayerid][listitem][pvSlotId]);
		mysql_tquery(MainPipeline, query, "OnQueryFinish", "ii", SENDDATA_THREAD, giveplayerid);
		...
		else
		{
			if(strlen(inputtext) >= 1)
			{
				...
				else
				{
					...
					{
						SendClientMessageEx(playerid, COLOR_GRAD2, "You don't have enough credits to purchase this item. Visit rg-rp.org to purchase credits.");
					}
					*/
					new namechangecost;
	...
	if(dialogid == MPSPAYTICKETS)
	{
		if(response)
		{
			...
			if(PlayerVehicleInfo[playerid][listitem][pvTicket] && PlayerVehicleInfo[playerid][listitem][pvImpounded] == 0) {
				...
				PlayerVehicleInfo[playerid][listitem][pvSpawned] = 0;
				PlayerVehicleInfo[playerid][listitem][pvPosAngle] = DMVRelease[rand][3];
				PlayerVehicleInfo[playerid][listitem][pvTicket] = 0;
		...
		if(response)
		{
			if(GetPlayerCash(playerid) >= 4000)
			{
				if(listitem == 0)
				{
					...
					{
						GivePlayerCash(playerid, -4000);
						SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: You have received 20 percent off this product. Instead of paying $4000, you paid $2000.");
					}
					else
					{
						GivePlayerCash(playerid, -2000);
			...
			if(GetPlayerCash(playerid) >= 5000)
			{
				if(listitem == 1)
				{
					...
					{
						GivePlayerCash(playerid, -5000);
						SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: You have received 20 percent off this product. Instead of paying $3000, you paid $5000.");
					}
					else
					{
						GivePlayerCash(playerid, -3000);
			...
			if(GetPlayerCash(playerid) >= 8000)
			{
				if(listitem == 2)
				{
					...
					if(PlayerInfo[playerid][pDonateRank] >= 1)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: You have received 20 percent off this product. Instead of paying $8000, you paid $5000.");
					}
					else
					{
						GivePlayerCash(playerid, -5000);
			...
			if(GetPlayerCash(playerid) >= 6000)
			{
				if(listitem == 3)
				{
					...
					if(PlayerInfo[playerid][pDonateRank] >= 1)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: You have received 20 percent off this product. Instead of paying $6000, you paid $4000.");
					}
					else
					{
						GivePlayerCash(playerid, -4000);
			...
			if(GetPlayerCash(playerid) >= 7000)
			{
				if(listitem == 4)
				{
					...
					if(PlayerInfo[playerid][pDonateRank] >= 1)
					{
						...
						SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: You have received 20 percent off this product. Instead of paying $7000, you paid $5000.");
					}
					else
					{
						GivePlayerCash(playerid, -5000);
...
/*	if(dialogid == MDC_CHECK && response)
	...
	else if (dialogid == DIALOG_MAKESLOTHOLDER && response)
	{
		if (PlayerInfo[playerid][pAdmin] >= 1337 || PlayerInfo[playerid][pFactionModerator] >= 2)
		{
			...
			PlayerInfo[iTargetID][pSlotHolder] = iGroupID;
			...
			GetPlayerName(iTargetID, name, sizeof(name));

			//arrGroupData[iGroupID][g_iMemberCount]++;
			format(string, sizeof(string), "You have been made the slot holder of the %s by Administrator %s.", arrGroupData[iGroupID][g_szGroupName], GetPlayerNameEx(playerid));
			SendClientMessageEx(iTargetID, COLOR_LIGHTBLUE, string);
			format(string, sizeof(string), "You have made %s the slot holder of the %s.", GetPlayerNameEx(iTargetID), arrGroupData[iGroupID][g_szGroupName]);
			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
			format(string, sizeof(string), "%s have made %s the slot holder of the %s.", GetPlayerNameEx(playerid), GetPlayerNameEx(playerid), arrGroupData[iGroupID][g_szGroupName]);
	...
	else if (dialogid == DIALOG_LISTLEADERS && response)
	{
		if (PlayerInfo[playerid][pAdmin] >= 4 || PlayerInfo[playerid][pASM] >= 1 || PlayerInfo[playerid][pFactionModerator] >= 2  || PlayerInfo[playerid][pSlotHolder] >= 0)
		{
			new
				iGroupID = listitem;
	...
	if(dialogid == DIALOG_MISCSHOP2 && response)
	{
		if(GetPVarInt(playerid, "MiscShop") == 1)
		{
			if(PlayerInfo[playerid][pCredits] < ShopItems[6][sItemPrice])
				return SendClientMessageEx(playerid, COLOR_GREY, "You don't have enough credits to purchase this item. Visit rg-rp.org to purchase credits.");
	...
	if(dialogid == DIALOG_MICROSHOP3)
	{
		...
		if(response)
		{
			...
			if(PlayerInfo[playerid][pCredits] < MicroItems[item]) return SendClientMessageEx(playerid, COLOR_GREY, "You don't have enough credits to purchase this item. Visit rg-rp.org to purchase credits.");
	...
	if(dialogid == DIALOG_TELEPORT)
	{
		if (response) {
			if(IsPlayerInAnyVehicle(playerid)) {
				SetVehiclePos(GetPlayerVehicleID(playerid), g_arrLocationData[listitem][e_LocationX], g_arrLocationData[listitem][e_LocationY], g_arrLocationData[listitem][e_LocationZ]);
			}
			else
			{
				SetPlayerPos(playerid, g_arrLocationData[listitem][e_LocationX], g_arrLocationData[listitem][e_LocationY], g_arrLocationData[listitem][e_LocationZ]);
				SetPlayerVirtualWorld(playerid, 0);
				SetPlayerInterior(playerid, 0);

				format(string, sizeof(string), "You have been teleported to %s", g_arrLocationData[listitem][e_LocationName]);
				SendClientMessageEx(playerid, COLOR_WHITE, string);
				format(string, sizeof(string), "~b~~h~%s", g_arrLocationData[listitem][e_LocationName]);
				GameTextForPlayer(playerid, string, 3, g_arrLocationData[listitem][e_LocationName]);
			}
		}
	}
	...
	if(dialogid == DIALOG_FIRESTATIONGARAGE)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
				    if(FireStationGarageDoor[0] == 1) return SendErrorMessage(playerid, "The gate is already opened.");
					MoveDynamicObject(FireStationGarage[0],1293.3500,-1357.8202,18.4354,0.6);
					SetTimer("FSGarageClose1", 20000, 0);
					SetTimer("StopPlayAudio", 5000, 0);
					FireStationGarageDoor[0] = 1;
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
					PlaySoundInArea(1153, 1293.3500,-1357.8202,18.4354, 20.0);
				}
				case 1:
				{
				    if(FireStationGarageDoor[1] == 1) return SendErrorMessage(playerid, "The gate is already opened.");
					MoveDynamicObject(FireStationGarage[1],1293.3500,-1367.3761,18.4347,0.6);
					SetTimer("FSGarageClose2", 20000, 0);
					FireStationGarageDoor[1] = 1;
					PlaySoundInArea(1153,1293.3500,-1367.3761,18.4347, 20.0);
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
				}
				case 2:
				{
				    if(FireStationGarageDoor[2] == 1) return SendErrorMessage(playerid, "The gate is already opened.");
					MoveDynamicObject(FireStationGarage[2],1293.3500,-1376.8995,18.4953,0.6);
					SetTimer("FSGarageClose3", 20000, 0);
					FireStationGarageDoor[2] = 1;
					PlaySoundInArea(1153,1293.34998,-1376.89954,14.49830,20.0);
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
				}
				case 3:
				{
				    GetDynamicObjectPos(FireStationGarage[0], X, Y, Z);
				    GetDynamicObjectPos(FireStationGarage[1], X, Y, Z);
				    GetDynamicObjectPos(FireStationGarage[2], X, Y, Z);
					MoveDynamicObject(FireStationGarage[0],1293.3500,-1357.8202,18.4354,0.6);
					MoveDynamicObject(FireStationGarage[1],1293.3500,-1367.3761,18.4347,0.6);
					MoveDynamicObject(FireStationGarage[2],1293.3500,-1376.8995,18.4953,0.6);
				...
				case 4:
				{
				    ...
				    GetDynamicObjectPos(FireStationGarage[1], X, Y, Z);
				    GetDynamicObjectPos(FireStationGarage[2], X, Y, Z);
					MoveDynamicObject(FireStationGarage[0],1293.34998,-1357.82019,14.49830,0.6);
					MoveDynamicObject(FireStationGarage[1],1293.34998,-1367.37610,14.49830,0.6);
					MoveDynamicObject(FireStationGarage[2],1293.34998, -1376.89954, 14.49830,0.6);
					PlaySoundInArea(1154, X, Y, Z, 20.0);
				}
			}
		}
	}
	if(dialogid == DIALOG_FIRESTATIONGARAGE2)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
				    GetDynamicObjectPos(FireStationGarage[3], X, Y, Z);
					MoveDynamicObject(FireStationGarage[3],1332.5734,-1357.8202,18.4623,1);
					SetTimer("FSGarageClose4", 20000, 0);
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
				}
				case 1:
				{
				    GetDynamicObjectPos(FireStationGarage[4], X, Y, Z);
					MoveDynamicObject(FireStationGarage[4],1332.5222,-1367.3761,18.4675,1);
					SetTimer("FSGarageClose5", 10000, 0);
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
					PlaySoundInArea(1153, X, Y, Z, 20.0);
				}
				case 2:
				{
				    GetDynamicObjectPos(FireStationGarage[5], X, Y, Z);
					MoveDynamicObject(FireStationGarage[5],1332.5734,-1376.8995,18.4999,1);
					SetTimer("FSGarageClose6", 20000, 0);
					SendServerMessage(playerid, "The garage door will automatically close in 20 seconds!");
					PlaySoundInArea(1153, X, Y, Z, 20.0);
				}
				case 3:
