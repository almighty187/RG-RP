...
public SetVehicleEngine(vehicleid, playerid)
{
    ...
    else if(engine == VEHICLE_PARAMS_OFF || engine == VEHICLE_PARAMS_UNSET)
	{
		...
		else SendClientMessageEx(playerid, COLOR_WHITE, "Vehicle engine started successfully (press ~k~~CONVERSATION_NO~ to turn the engine off).");
	...
	if(GetPVarInt(playerid, "HelpTime") > 0)
 	{
    	...
    	if(GetPVarInt(playerid, "HelpTime") == 0)
     	{
      		SendClientMessageEx(playerid, COLOR_GREY, "Your help request has expired. Its recommended you seek help on the forums (forum.rg-rp.org)");
...
stock IsInvalidSkin(skin) {
	...
	if(skin < 0 || skin > 311 && skin < 20000 || skin > 30001) return 1;
    return 0;
}
